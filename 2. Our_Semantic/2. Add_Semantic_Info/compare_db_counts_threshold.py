...
# 여기서 임계값을 설정할 수 있습니다. 예를 들어, 0.5로 설정하면 IoU가 0.5 이상인 경우에만 RLE 마스크 IoU를 계산합니다.
IOU_THRESHOLD = 0.5
...
def compare_objects(obj1, obj2):
    ...
    # 마스크 IoU를 계산하기 전에 바운딩 박스 IoU 점수가 임계값보다 높은지 확인합니다.
    if bbox_iou_score >= IOU_THRESHOLD:
        mask_iou_score = rle_mask_iou(obj1['segmentation'], obj2['segmentation'], obj1['segmentation']['size'])
    else:
        mask_iou_score = 0  # 임계값 미만인 경우, 마스크 IoU 점수는 0으로 설정합니다.
    
    return {
        'area_diff': area_diff,
        'bbox_iou': bbox_iou_score,
        'mask_iou': mask_iou_score
    }
